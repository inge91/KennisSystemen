...
% you get:
% b+S------a+S------a+E------b+E
% The +S give the starting points and +E the ending points
%
% exemple case:
% [a, voor ,b]
% [a, voor ,c]
% [d, tijdens c]
%
% Note there is a visual output for the interval just like assignment 1
% type solve.
...
solve:-
    %find all points for timeline
    findall(X,na(start,X),List),
    unique_points(List,List2),
    findall(X, timelines(List2,[]), _),
    printALL,!.

timelines([],[_|T]):-
     %count elements as a check there are no doubles no missing
     findall(X,na(start,X),List),
     unique_points(List,List2),
     flatten(T,T_flatten),
     length(List2,L),
...
timelines(X,[]):-
    %add the start element to the time line as starting point
    timelines(X,[start]).

timelines(PossiblePoints, Solution):-
    %findall the point for wich we are sure has no elements before him that are not in the solution already
    findall(A, (member(A,PossiblePoints), findall(Test,((na(Test,A)), flatten(Solution,Test2), \+member(Test,Test2)), [])),Uitkomst),
    %select one of the possible points
    member(A, Uitkomst),
    %check if other points start at the same time
    ...
    %and retract them from the possible points
